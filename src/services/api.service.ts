...
export class ApiService {
  ...
  private _settings = signal<{ categories: string[]; destinations: string[], outgoingSubcategories: string[] }>({
    categories: ['Médicaments', 'Fournitures', 'Consommables', 'Autres'],
    destinations: ['Fournisseur A', 'Dépôt Central', 'Service 1', 'Périmé'],
  ...
  addArticle(article: Omit<Article, 'id' | 'createdAt' | 'updatedAt' | 'priceHistory'>): Article {
    ...
    if (existingArticles.some(a => this.areSimilar(a.name, article.name) && a.unit === article.unit)) {
  ...
  // Settings
  updateSettings(key: 'categories' | 'destinations' | 'outgoingSubcategories', value: string[]): { categories: string[], destinations: string[], outgoingSubcategories: string[] } {
    const oldSettings = this._settings();
    const oldList = oldSettings[key];
    const deletedItems = oldList.filter(item => !value.includes(item));
    
    if (deletedItems.length > 0) {
      if (key === 'categories') {
        const categoryInUse = deletedItems.find(cat => this._articles().some(a => a.category === cat));
        if (categoryInUse) {
          throw new ApiError('CATEGORY_IN_USE', { item: categoryInUse });
        }
      }
      if (key === 'destinations') {
        const destinationInUse = deletedItems.find(dest => this._movements().some(m => m.supplierDest === dest));
        if (destinationInUse) {
          throw new ApiError('DESTINATION_IN_USE', { item: destinationInUse });
        }
      }
       if (key === 'outgoingSubcategories') {
        const subcategoryInUse = deletedItems.find(sub => this._movements().some(m => m.subcategory === sub));
        if (subcategoryInUse) {
          throw new ApiError('SUBCATEGORY_IN_USE', { item: subcategoryInUse });
        }
      }
    }
