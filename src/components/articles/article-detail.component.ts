...
@Component({
  ...
  standalone: true,
  imports: [ModalComponent, CustomDatePipe, ReactiveFormsModule],
  template: `
      ...
      @if (article(); as art) {
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 transition-all" [class.blur-sm]="isMovementModalOpen()">
          ...
          <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-slate-50 p-4 rounded-lg border">
              ...
              <ul class="space-y-1 text-sm max-h-40 overflow-y-auto pr-2">
                @for(history of art.priceHistory.slice().reverse(); track history.date + history.price) {
          ...
          <div class="lg:col-span-2">
            ...
            <div class="overflow-y-auto max-h-[65vh] border rounded-lg bg-white">
              <table class="w-full text-sm text-left">
                ...
                <tbody>
                  @for (movement of articleMovements(); track movement.id) {
    ...
    <app-modal [isOpen]="isMovementModalOpen()" [title]="t().movements.recordMovement" (close)="closeMovementModal()">
        <form [formGroup]="movementForm" (ngSubmit)="onAddMovementSubmit()" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label for="modal-supplierDest" class="block mb-2 text-sm font-medium text-slate-900">{{ t().movements.form.supplierDest.label }}</label>
                    <select id="modal-supplierDest" formControlName="supplierDest" class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5">
                        <option value="">{{ t().movements.form.supplierDest.placeholder }}</option>
                        @for (dest of destinations(); track dest) {
                            <option [value]="dest">{{ dest }}</option>
                        }
                    </select>
                </div>
                @if (movementForm.get('type')?.value === 'Sortie') {
